...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  late final List<String> _bookedSeats;

  @override
  void initState() {
    super.initState();
    _bookedSeats = _generateBookedSeats(widget.event['id'] ?? '');
  }

  List<String> _generateBookedSeats(String eventId) {
    ...
    // Tạo mã băm deterministic từ eventId để tạo sơ đồ ghế khác nhau cho mỗi sự kiện
    int hash = 0;
    for (int i = 0; i < eventId.length; i++) {
      hash = eventId.codeUnitAt(i) + ((hash << 5) - hash);
    }
    
    final List<String> booked = [];
    // Số lượng ghế đã đặt dao động ngẫu nhiên từ 6 đến 14 ghế tùy theo sự kiện
    final int numBooked = 6 + (hash.abs() % 9);
    
    for (int i = 0; i < numBooked; i++) {
      final int seatIndex = (hash.abs() + i * 17) % allSeats.length;
      final String seat = allSeats[seatIndex];
